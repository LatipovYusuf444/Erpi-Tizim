...
export default function Navbar3() {
  return (
    <div className="mx-auto container">
      <nav className="w-full max-w-[1402px] h-auto flex flex-wrap gap-2 border border-[#334F9D] rounded-3xl px-3 py-2 bg-muted mt-4">
        {tabs.map((t) => (
          <NavLink
            key={t.label}
            to={t.to}
            ...
            className={({ isActive }) =>
              cn(
                ...
                !isActive && "text-black",
                isActive && "bg-gradient-to-r from-[#1C96C8] to-[#334F9D] text-white"
              )
            }
          >
            {t.label}
